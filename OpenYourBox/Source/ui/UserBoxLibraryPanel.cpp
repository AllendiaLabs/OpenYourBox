...
void UserBoxLibraryPanel::renderInsertMenu(
    library::UserBoxLibrary &library,
    const std::function<juce::ValueTree(const juce::String &, juce::String &)>
        &loadSnapshot,
    const std::function<void(const juce::String &, std::int32_t)> &onInsert) {
  const auto renderMembers = [&](auto &&self, const juce::ValueTree &snapshot,
                                 std::int32_t rootId,
                                 const library::UserBoxLibraryEntry &entry) -> void {
    std::unordered_map<std::int32_t, juce::ValueTree> groups;
    std::unordered_map<std::int32_t, juce::ValueTree> nodes;
    for (const auto child : snapshot) {
      if (child.hasType("Group"))
    ...
    for (const auto memberId : memberIds) {
      ...
      if (group != groups.end()) {
        const auto label =
            group->second.getProperty("name", "Group").toString();
        if (ImGui::BeginMenu(label.toRawUTF8())) {
          if (ImGui::MenuItem("Insert group"))
            onInsert(entry.id, memberId);
          self(self, snapshot, memberId, entry);
          ImGui::EndMenu();
        }
      } else {
        const auto node = nodes.find(memberId);
        if (node != nodes.end()) {
          const auto label = node->second["label"].toString();
          if (ImGui::MenuItem(label.toRawUTF8()))
            onInsert(entry.id, memberId);
        }
      }
      ImGui::PopID();
  ...
  const auto renderFolderMenu = [&](auto &&self, const juce::String &folder) -> void {
    const auto children = childFolders(library.getFolders(), folder);
    std::vector<const library::UserBoxLibraryEntry *> entries;
    for (const auto &entry : library.getEntries()) {
      if (entry.folder == folder)
    ...
    for (const auto &child : children) {
      const auto childPath = folder.isEmpty() ? child : folder + "/" + child;
      ImGui::PushID(childPath.toRawUTF8());
      if (ImGui::BeginMenu(child.toRawUTF8())) {
        self(self, childPath);
        ImGui::EndMenu();
      }
      ImGui::PopID();
    ...
    for (const auto *entry : entries) {
      ...
      const auto groupEntry = entry->kind == library::UserBoxKind::group;
      if (groupEntry) {
        if (ImGui::BeginMenu(entry->name.toRawUTF8())) {
          if (ImGui::MenuItem("Insert box"))
            onInsert(entry->id, 0);
          juce::String error;
          juce::ValueTree snapshot;
          if (loadSnapshot)
            snapshot = loadSnapshot(entry->id, error);
          if (snapshot.isValid())
            renderMembers(renderMembers, snapshot,
                          static_cast<std::int32_t>(snapshot["rootId"]), *entry);
          ImGui::EndMenu();
        }
      } else if (ImGui::MenuItem(entry->name.toRawUTF8())) {
        onInsert(entry->id, 0);
      }
      ImGui::PopID();
    }
