...
### Session 2026-09-05 (post-implement refinements)

- Q: Where do Loss weights live? → A: **On the Train stage schedule only** (per loss entry in a stage). Loss boxes expose `loss_type` (and type-specific properties), **not** a weight property. Empty schedule → one stage of all wired losses at weight **1.0**.
- Q: May Data Loader attach to an empty pin (no live cable yet)? → A: **Yes**. Refuse only when a **processing** (non–Audio-In / non–group-hub) live upstream already drives the pin. Audio In (or group-input hub) + Data Loader coexistence remains allowed.
- Q: How may Loss pins be wired? → A: **Data Loader → Loss target only**; **live/path → Loss prediction only**. Refuse Data Loader on prediction and live cables on target.
- Q: When grouping a member that already has both Audio In and Data Loader on the same input pin, how many group input hubs? → A: **One hub pin** mediating both external cables (dedupe by member pin), with a single interior hub→member cable.
- Q: Can RAVE-style stage-2 encoder freeze happen in one Run? → A: **Yes, via per-stage freeze**. Each loss stage may list `freeze_element_ids`. Train UI: optional (collapsed) freeze tree matching Project Structure; unarmed elements are force-checked and disabled; checking a group checks all descendants; partial child selection shows a mixed (minus) parent checkbox.
...
3. **Given** a pin that is empty **or** already has a live Audio In / group-hub connection, **When** the user connects a data-loader output to that pin, **Then** the connection is allowed (live and training feeds coexist when both are present)
3b. **Given** a pin already driven by an upstream processing node, **When** the user tries to attach a data loader, **Then** the connection is refused with a clear reason
4. **Given** a chain A→B→C where A’s external input already has a data-loader feed, **When** the user tries to attach a data loader to B’s or C’s input pin that is fed by A, **Then** the connection is refused with a clear reason (internal graph pins cannot take a data loader)
...
The designer adds **loss nodes** and connects them to the architecture: **prediction** from the live/path graph output (or intermediate), and **target** from a Data Loader output only. Multiple losses may be active with **per-loss weights on the Train stage schedule** (weighted sum). Training configurations define an ordered **loss stage schedule** (different loss sets and/or weights over successive step ranges)—enough to reproduce RAVE/reconstruction-style multi-term and multi-stage training without a built-in architecture mode. Same-data vs different-data intent follows from what the data loader supplies on input vs target wires. Training also exposes a general hyperparameter surface; the user can save and load named training configurations in a **user training-config library** and as **project config snapshots**. **No regression**: every creative training outcome previously available under prior specs (including mapping-style and reconstruction-style results and equivalent recipe behaviors) remains achievable via the general graph + data loader + loss + training-configuration model.
...
1. **Given** the palette, **When** the user adds a loss node, **Then** they can connect **prediction** from a live/path output and **target** from a Data Loader only
...
3. **Given** multiple validly wired loss nodes with stage weights, **When** training Runs in a single stage (or an implicit empty-schedule stage at weight 1.0), **Then** the optimized objective is the weighted sum of those losses
3b. **Given** the user attempts Data Loader → Loss prediction or live → Loss target, **When** they connect, **Then** the connection is refused
...
5. **Given** the need to reproduce prior reconstruction-style (RAVE-like) multi-term/multi-stage training, **When** the user configures loss types, stage weights, and stages plus data loader materials, **Then** they can express that recipe fully in the general model (capability-class parity)
6. **Given** the Train panel, **When** the user inspects training controls, **Then** they can edit a general hyperparameter set sufficient to express previously built-in recipe differences (steps, learning-related settings, stage loss weights/schedules, schedule/regularization options that the product exposes)—without a reconstruction/mapping mode control and without a weight control on the Loss box itself
6b. **Given** a multi-stage schedule, **When** the user expands the per-stage **Freeze** tree (collapsed by default), **Then** they see Project Structure with checkboxes; unarmed boxes are checked and disabled; checking a group checks all sub-boxes; a partially checked group shows mixed state
6c. **Given** stage 1 with encoder armed and stage 2 with encoder in `freeze_element_ids`, **When** training Runs as one job, **Then** encoder parameters update only during stage 1 and stay frozen during stage 2
...
4. **Given** a member pin that already has both a live Audio In and a Data Loader cable, **When** the user groups that member, **Then** the group exposes **one** input hub pin that receives both external cables (not two hubs for the same member pin)
...
- What happens when multiple losses are wired but no stage schedule? Training uses a single implicit stage: every validly wired loss at weight **1.0** for `total_steps`.
- What happens when the user sets stage weights in the Train panel? Those weights are the sole loss multipliers (Loss boxes have no weight property).
- What happens when Data Loader is attached to an empty pin? Allowed; a later live Audio In may coexist. A later processing live upstream on that pin is a separate connect-time concern for further Data Loader attaches.
- What happens when Data Loader targets Loss prediction, or live targets Loss target? Connection refused.
- What happens when live Audio In and data loader both feed the same pin, then the member is grouped? One group input hub mediates both external cables.
...
- **FR-006**: System MUST allow a data-loader connection to an **empty** input pin or to a pin already fed by **Audio In** (or a **group-input** hub carrying that feed), including when a live cable already exists. System MUST NOT allow a data-loader connection on a pin that is already driven by an upstream **processing** node. In a chain A→B→C, if A is data-loader-fed on its external input, B and C MUST NOT accept data-loader connections on pins fed by A. For a path to be trained, every external source input required by that path MUST have a data-loader feed (audio list and/or constant/scalar utility); otherwise Start training MUST fail with a clear message.
...
- **FR-009**: System MUST provide loss nodes that users connect as follows: **prediction** pin accepts **live/path** connections only; **target** pin accepts **Data Loader** connections only. Multiple validly wired losses MUST be combinable via **per-loss weights on the stage schedule** (weighted sum). Training configurations MUST support ordered **loss stage schedules** (changing active losses and/or weights over step ranges). Each stage MAY list **`freeze_element_ids`** so armed on-path elements can be frozen for that stage only (RAVE-like encoder freeze in one Run). Loss boxes MUST NOT expose a weight property. Together with the loss catalog and hyperparameters, this MUST be sufficient to reproduce RAVE/reconstruction-style multi-term and multi-stage training without a built-in architecture mode.
- **FR-009a**: The Train panel stage editor MUST expose an optional **Freeze** structure tree (collapsed/hidden by default) listing **only armable/trainable** elements (and groups that contain them). Disarmed armable elements MUST appear checked, disabled, and visually gray. Checking a group MUST check all descendant armable boxes. A group with a mix of checked/unchecked children MUST show a mixed (indeterminate / minus) checkbox state. On the Train tab, when every armable element under a group is drawn transparent, that group box MUST also be drawn transparent.
- **FR-010**: System MUST refuse to start training when no usable loss wiring is present, with a clear user-visible reason.
- **FR-011**: System MUST expose a general training parameter surface (hyperparameters and related train settings, including **stage** loss weights and schedules) sufficient for users to configure runs that reproduce prior built-in recipe behaviors without those recipes being hard-coded product modes.
- **FR-012**: Users MUST be able to save and load named training configurations that capture those hyperparameters and related train settings (including stage loss weights and schedules), in **both** forms: (1) a **user training-config library** reusable across projects/sessions, and (2) a **project config snapshot** stored with the project.
- **FR-013**: Users MUST be able to create a group from a selection of exactly one allowed box (Audio I/O exclusion unchanged). When a member pin already has both a live Audio In and a Data Loader cable, grouping MUST create **one** group input hub for that member pin (both external cables attach to that hub; one interior hub→member cable)—not one hub per crossing link.
...
- **Loss Node**: Graph element that defines supervision; **prediction** from live/path, **target** from Data Loader; no on-box weight. Ignored during live playback.
- **Loss Stage Schedule**: Ordered list of training stages; each stage specifies duration, `{ loss_node_id, weight }` entries, and optional `freeze_element_ids`. Empty schedule ⇒ all wired losses at weight 1.0 for `total_steps` (no stage freezes).
- **Trainable Subgraph**: Nodes that are on the active data-loader path **and armed**—their parameters are updated during training (unless frozen for the current stage).
...
- **SC-013**: Guided review confirms that attempting to attach a data loader to a processing-driven pin is refused 100% of the time, while attaching to an empty pin or a pin fed by Audio In / group-input hub remains allowed.
- **SC-020**: Guided review confirms Data Loader→Loss prediction and live→Loss target are refused 100% of the time; Data Loader→target and live→prediction remain allowed.
- **SC-021**: When a member with shared Audio In + Data Loader on one pin is grouped, exactly one group input hub exists for that pin in 100% of guided checks.
...
- **SC-022**: In a guided two-stage run that freezes an armed encoder only in stage 2, encoder weights change in stage 1 and remain unchanged in stage 2 in 100% of checks.
...
- Loss nodes are first-class palette elements **without** on-box weights; training configurations include optional **loss stage schedules** with per-entry weights (default 1.0 when schedule empty). Exact loss type catalog MUST cover what prior reconstruction/RAVE and mapping recipes needed—capability-class parity as the acceptance bar.
- Group-of-one does not change Audio I/O exclusion; hub creation MUST dedupe by member pin when live and data-loader cables share that pin.
