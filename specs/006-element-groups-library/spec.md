...
- What if the declared input and output lane counts differ, no directed path connects the hubs, or a serial copy cannot feed the next after applying per-copy properties (including a residual join that cannot combine)? Requested N remains persisted, but effective runtime copies is 1. The group shows the reason and relevant shape values; candidate shape-driving properties are highlighted where available. Once the user repairs the condition, N reactivates automatically. First-copy output need not match first-copy input: encoder/decoder stacks with per-copy channel or stride lists are legal when each copy can feed the next.
...
- **FR-017b**: N &gt; 1 is active only when the group declares equal, nonzero Group Input and Group Output lane counts, contains a directed path from the input hub to the output hub, and each serial copy can feed the next after applying that copy’s properties (per-copy `channels`/`features`/`stride` lists and `in` bindings). First-copy output need not match first-copy input. A residual join that cannot combine at any copy keeps N inactive. Otherwise the requested N MUST remain stored while effective runtime copies clamps to 1, with no partial/orphan runtime chain.
